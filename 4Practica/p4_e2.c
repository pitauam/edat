...
int main(int argc, char **argv) {
    ...
    /* 1. Inicializar Radio y leer canciones del fichero */
    r = radio_init();
    f = fopen(argv[1], "r");
    if (!f || !r) {
    ...
    /* 2. Inicializar el Árbol con la función de comparación por ID */
    /* Nota: Usamos music_plain_print para las salidas de texto */
    ...
    /* SALIDA ESPERADA: Estado Inicial */
    printf("Número de nodos inicial: %ld\n", tree_size(tree));
    printf("Recorrido InOrder inicial:\n");
    tree_inOrder(stdout, tree);

    /* 4. Buscar y eliminar la canción */
    printf("\nEliminando canción con ID: %ld...\n", id_to_remove);
    
    /* Creamos un objeto temporal con el ID para buscarlo en el árbol */
